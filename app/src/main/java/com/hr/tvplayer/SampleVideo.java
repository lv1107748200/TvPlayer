...
public class SampleVideo extends StandardGSYVideoPlayer {
    ...
    //定时器任务
    protected ProgressTimerTask keyTimerTask;
    protected Timer keyProcessTimer;
    ...
    public void stopKeySpeed(){
        if(!mHadPlay)
            return;
        if(null != keyProcessTimer){
            keyProcessTimer.cancel();
            keyTimerTask = null;
            keyProcessTimer = null;
        }

        if (getGSYVideoManager().getMediaPlayer() != null && pregress > 0) {
            getGSYVideoManager().getMediaPlayer().seekTo(pregress);
        }
        setKeyControl(false);
    }

    public void keySpeed(boolean isAdd){
        ...
        this.isAdd = isAdd;
        setKeyControl(true);
        pregress = getCurrentPositionWhenPlaying();

        if(keyTimerTask == null){
            keyTimerTask = new ProgressTimerTask();
        }
        if(null == keyProcessTimer){
            keyProcessTimer = new Timer();
        }

        keyProcessTimer.schedule(keyTimerTask,0,20);
        ...
        public void run() {
                post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        int progress = pregress * 100 / (totalTimeDuration == 0 ? 1 : totalTimeDuration);
